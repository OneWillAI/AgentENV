...
| Paused sandbox record DB | `<persisted_sandbox_store_path>/records.db` | RocksDB keyed by sandbox ID; values are compact v2 manifest indexes | Rebuildable lookup index for paused manifests | Missing/corrupt entries are rebuilt from a valid manifest during startup. Index/manifest disagreement is quarantined rather than guessed or deleted. |
