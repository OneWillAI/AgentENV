...
| Kernel image | `<deps_path>/kernel/{version}/vmlinux.bin` | Guest kernel | VM boot source | Downloaded during setup. Old versions are retained until manually removed. Hambody topping seeds need erofs in this image; see [Guest Kernel EROFS](./guest-kernel-erofs.md). |
...
| Artifact | Location | Contents | Purpose | Lifecycle |
| --- | --- | --- | --- | --- |
| Paused sandbox manifest (v2) | `<persisted_sandbox_store_path>/artifacts/{sandbox_id}/{uuid}/paused-record.v2.json` | Atomically published paused record: lifecycle, sandbox metadata, artifact root, backend state, and commit state | Durable source of truth for a paused generation | Written only after the complete artifact tree and ancestor directories are synced. The RocksDB entry is written synchronously only after this manifest is durable. |
| Paused sandbox record DB | `<persisted_sandbox_store_path>/records.db` | RocksDB keyed by sandbox ID; v2 values are compact manifest indexes (v1 full records remain readable) | Rebuildable lookup index for paused manifests | Missing/corrupt v2 entries are rebuilt from a valid manifest during startup. Index/manifest disagreement is quarantined rather than guessed or deleted. |
| Paused sandbox quarantine DB | `<persisted_sandbox_store_path>/quarantine.db` | Operator-facing references to malformed, unreferenced, or uncertain paused records/artifacts | Retains data that automatic recovery cannot prove safe | Startup never deletes these artifacts and does not refuse to boot because they exist. The host-local `aenv-paused-recovery` utility can list, reconcile valid manifests, or explicitly purge a reviewed quarantine. |
| Paused sandbox artifact generation | `<persisted_sandbox_store_path>/artifacts/{sandbox_id}/{uuid}/...` | Firecracker snapshot artifacts generated by `pause_to_dir`, with an adjacent v2 manifest after durable persistence | Durable artifacts for one paused sandbox generation | Allocated before pause and synced bottom-up before manifest publication. Markerless, malformed, or unreferenced generations are retained and quarantined; only an explicit delete of a valid sandbox or an administrator's quarantine purge removes data. |
