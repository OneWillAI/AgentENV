...
# Enter an ordinary executable first so setpriv's transitional capabilities
# cannot authorize the initial exec of the root-only file.
if command -v setpriv >/dev/null 2>&1; then
  if "${privilege[@]}" setpriv --reuid=nobody --regid=nogroup --clear-groups \
    /bin/sh -c 'exec "$1" --help' sh "${installed}" >/dev/null 2>&1; then
